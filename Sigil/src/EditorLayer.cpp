...
namespace Pressure
{
    ...
    void EditorLayer::OnImGuiRender()
    {
		...
		if (selectedEntity && m_GizmoType != -1)
		{
			...
			IMGUIZMO_NAMESPACE::SetRect(m_ViewportBounds[0].x, m_ViewportBounds[0].y, m_ViewportBounds[1].x - m_ViewportBounds[0].x, m_ViewportBounds[1].y - m_ViewportBounds[0].y);
	...
	void EditorLayer::OnOverlayRender()
	{
		...
		if (m_ShowPhysicsColliders)
		{
			// Box Colliders
			{
				...
				for (auto e : view)
				{
					auto [tc, collider] = view.get<TransformComponent, BoxCollider2DComponent>(e);

					glm::vec3 translation = tc.Translation + glm::vec3(collider.Offset, 0.001f);
			...
			// Circle colliders
			{
				...
				for (auto e : view)
				{
					auto [tc, collider] = view.get<TransformComponent, CircleCollider2DComponent>(e);
	...
	{
		PRS_CORE_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate);
